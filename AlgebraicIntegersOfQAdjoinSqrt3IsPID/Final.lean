...
lemma coeff_fq : coeff fq 2 = 1 ∧ coeff fq 1 = 0 ∧ coeff fq 0 = 3 := by sorry
...
lemma mem_K_minpoly (x : K) : ∃ a b : ℚ, (AdjoinRoot.mk fq) ((monomial 1) b + C a) = x ∧ minpoly ℚ x = (monomial 2 1 - monomial 1 (2 * a) + C (a ^ 2 + 3 * b ^ 2)) := by
  ...
  · have nat_to_rat : (1 : ℚ) = (1 : AdjoinRoot fq) := by simp
    ...
    rw [fq] at root_of
    --simp only [map_ofNat, coeff_add, ite_true, coeff_ofNat_succ, add_zero, one_smul,
    --  OfNat.one_ne_ofNat, ite_false, zero_smul, OfNat.zero_ne_ofNat, coeff_ofNat_zero, zero_add] at root_of
    simp at root_of
  ...
  · rw[deg_fx]
    ...
    have ne_zero : C c1 * (C a + C b * X) + C c0 ≠ 0 := by sorry
