...
structure K_eq_eisenstein (K : Type*) (eisensteinInt : Type*)  [Mul eisensteinInt] [Add K] [Add eisensteinInt] extends Equiv :
  Type* where
...
map_mul' : ∀ (x y : NumberField.ringOfIntegers K), Equiv.toFun self.toEquiv (x * y) = Equiv.toFun self.toEquiv x * Equiv.toFun self.toEquiv y := by
  intros
  ext <;> simp <;> ring
map_add' : ∀ (x y : NumberField.ringOfIntegers K), Equiv.toFun self.toEquiv (x + y) = Equiv.toFun self.toEquiv x + Equiv.toFun self.toEquiv y := by
  intros
  ext <;> simp <;> ring
